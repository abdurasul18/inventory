...
async function getList() {
  loading.value = true;
  try {
    const { data } = await ItemTypeService.getList(payload.value);
...
let currentItem = ref<IItemType>()
async function addItem(item: any) {
  addLoading.value = true
  try {
    if (mode.value == 'create') {
      let res = await ItemTypeService.create(item)

    }
    else {
      let res = await ItemTypeService.update(currentItem.value?.id, item)
    }
    addShow.value = false
...
async function deleteItem() {
  ...
  loading.value = true
  try {
    await ItemTypeService.delete(currentItem.value?.id)
    getList()
    ...
    <Dialog staticBackdrop :open="addShow" @close="addShow = false">
      <Dialog.Panel class="px-5 py-10">
        <Loading :active="loading">
          <AddUpdateItemType @close="addShow = false" @success="addItem" :item="currentItem" :mode="mode" />
        </Loading>
      </Dialog.Panel>
    </Dialog>
    <!-- END: Modal Content -->

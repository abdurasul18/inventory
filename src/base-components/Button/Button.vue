...
<template>
  <component :is="as" :class="computedClass" v-bind="_.omit(attrs, 'class')" :disabled="$props.loading">
    <LoadingIcon icon="tail-spin" v-if="$props.loading" class=" text-white" color="#fff"/>
    <slot v-else></slot>
  </component>
</template>
